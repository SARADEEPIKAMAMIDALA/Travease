...
const UserSearchByDuration = () => {
  const [fromDuration, setFromDuration] = useState("");
  const [toDuration, setToDuration] = useState("");
  const [filteredTours, setFilteredTours] = useState([]);

  const handleSearch = () => {
    const from = parseInt(fromDuration);
    const to = parseInt(toDuration);
    if (!from || !to || from > to) {
      alert("Enter valid duration range.");
      return;
    }

    const results = tourPackages.filter(pkg => pkg.days >= from && pkg.days <= to);
    setFilteredTours(results);
  };

  return (
    <Container className="my-5">
      <h2 className="text-center mb-4">Search Tours by Duration</h2>
      <Row className="mb-4">
        <Col md={4}>
          <Form.Control
            type="number"
      ...
      <Row>
        {filteredTours.length > 0 ? (
          ...
          <Col>
            <p className="text-center text-muted">No tours in this duration range.</p>
          </Col>
        )}
      </Row>
    </Container>
...
export default UserSearchByDuration;
